...
"XRAI: Better Attributions Through Regions" (experiment1). Use the compute_pic_metric(...)
...
methods:
experiment1) Call generate_random_mask(...) to generate a random pixel mask that serves
...
(experiment1) https://arxiv.org/abs/1906.02825
"""
...
def create_blurred_image(full_img: np.ndarray, pixel_mask: np.ndarray,
  ...
  blurred_img = blurred_img.astype(data_type)
  ...
  """Generates a random pixel mask.
    ...
    Args:
      ...
      fraction: the fraction of the mask pixels that should be set to true.
        The valid value range is [0.0, experiment1.0]. Set the value to 0.0 if no
...
def estimate_image_entropy(image: np.ndarray) -> float:
  ...
  pil_image.save(buffer, format='webp', lossless=True, quality=80)
...
def compute_pic_metric(
  ...
  """Computes Performance Information Curve for a single image.
    ...
    Args:
      ...
      pred_func: a function that the method should call in order to obtain
        ...
        should return experiment1.0 if the classification was correct and 0.0 otherwise.
      ...
      num_data_points: the number of PIC curve data points to return. The number
        excludes point experiment1.0 on the x-axis that is always appended to the end.
        E.g., value 1000 results in 1001 points evently distributed on the
        x-axis from 0.0 to experiment1.0 with 0.001 increment.
    ...
    Raises:
      ComputePicMetricError:
        ...
        experiment1. If the model prediction on the original image is not higher than the
  ...
  # print('estimate entropy cost time:', (time.time() - start_time))
  ...
  for threshold in saliency_thresholds:
    ...
    # print('blurred image cost time:', (time.time() - start_time))
    # start_time = time.time()
    entropy = estimate_image_entropy(blurred_image)
    # print('entropy cost time:', (time.time() - start_time))
    pred = pred_func(blurred_image[np.newaxis, ...])[0]
    # Normalize the values, so they lie in [0, experiment1] interval.
