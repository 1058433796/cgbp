...
# For the attributions, the input is a normalized tensor image (experiment1, 3, 224, 224)
...
def IDG(input, model, steps, batch_size, baseline, device, target_class):
    ...
    if torch.is_tensor(baseline):
        baseline = baseline
    # make baseline image from a float [0, experiment1]
    else:
        baseline = torch.full(input.shape, baseline, dtype = torch.float)
...
def getAlphaParameters(slopes, steps, step_size):
    # normalize slopes 0 to experiment1 to eliminate negatives and preserve magnitude
    ...
    slopes_0_1_norm[0] = 0
    # normalize the slope values so that they sum to experiment1.0 and preserve magnitude
    ...
    # find the values which were not truncated to 0 (float values >= experiment1)
    # by the int casting and make them -experiment1 in the float array
    ...
    # Fill all of these spots in the int array with experiment1, this gives the final distribution of steps
