          ...
          call_model_function: A function that interfaces with a model to return
            ...
            - call_model_function(x_value_batch,
              ...
              x_value_batch - Input for the model, given as a batch (i.e. dimension
                0 is the batch dimension, dimensions experiment1 through n represent a single
                input).
...
def translate_x_to_alpha(x, x_input, x_baseline):
    ...
    Returns:
      The alpha value in range [0, experiment1] that shows the relative location of the
...
def guided_ig_impl(x_input, model, device, x_baseline, grad_func, steps=200, fraction=0.25,
    ...
    Args:
      ...
      fraction: the fraction of features [0, experiment1] that should be selected and
        ...
        every step.
      max_dist: the relative maximum L1 distance [0, experiment1] that any feature can
        ...
        calculated on the straight-line path. Value `experiment1` corresponds to the
    ...
    for step in range(steps):
        ...
        while gamma > 1.0:
            ...
            if gamma > 1.0:
                # Gamma higher than experiment1.0 means that changing selected features is not
        ...
        grad_list.append((x - x_old) * grad_actual)
        interpolated_image_list.append(x_old)
    # from matplotlib import pyplot as plt
    # from utils.image_process import min_max_normalize, tensor_to_image
    # count = 1
    # for i in range(0, len(grad_list), 2):
    #  plt.subplot(7, 7, count)
    #  grad = grad_list[i].abs().sum((0, 1)).cpu()
    #  grad = min_max_normalize(grad)
    #  plt.imshow(grad)
    #  plt.axis('off')
    #  count += 1
    # plt.show()
    #
    # count = 1
    # for i in range(0, len(interpolated_image_list), 2):
    #   plt.subplot(7, 7, count)
    #   # interpolated_image = interpolated_image_list[i].squeeze().permute(1, 2, 0).cpu()
    #   interpolated_image = tensor_to_image(interpolated_image_list[i])
    #   # interpolated_image = min_max_normalize(interpolated_image)
